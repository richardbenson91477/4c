...
const struct func_info _func_p_info [_4C_FUNC_P_ID_N + 1] = {
    {"print-i", 7, "_4c_func_print_i", 16, type_id_nil, 1, {type_id_int}},
    {"add-i", 5, "_4c_func_add_i", 14, type_id_int, 2, {type_id_int, type_id_int}},
    {"set-i", 5, "_4c_func_set_i", 14, type_id_nil, 1, {type_id_nil}},
...
bool func_validate_args (struct func_info *_fi, struct syntax_tree *_st) {
    ...
    if (_st->nodes_a.n != _fi->arg_n) {
        fprintf(stderr, "error: func_validate_args: argument counts unequal\n");
        return false;
    }
    ...
    for (uint32_t c = 0; c < _st->nodes_a.n; c++) {
        ...
        if (_st2->ti.subtype_id != _fi->arg_type_ids[c]) {
            fprintf(stderr, "error: func_validate_args: type mismatch: \"%s\" is not \"%s\"\n",
                type_id_syms[_st2->ti.subtype_id], type_id_syms[_fi->arg_type_ids[c]]);
            return false;
        }
    }
